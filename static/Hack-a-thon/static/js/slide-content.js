...
CanvasDemo.prototype.drawPixels = function() {
  ...
  for (var i = 0; i < id_old.data.length; i++) {
    if (i % 4 != 3) {
      id_new.data[i] = 255 - id_old.data[i];
    } else {
      id_new.data[i] = id_old.data[i];
    }
  }
...
NotificationManager.prototype.show = function() {
  ...
  if (window.webkitNotifications.checkPermission() == 0) {
    window.webkitNotifications.createNotification(
      '/static/images/frostydrink.gif', 
      'It\'s Beer O\'Clock!', 
      'You\'ll find me at the Thirsty Bear as soon as we\'re done!').show();
  } else {
    this.request();
  }
};
